...
HF_TOKEN = os.getenv("HF_TOKEN")  # на всякий случай, но для public space может не понадобиться
...
async def generate_image_flux(image_path: str, prompt: str) -> str:
    """
    Генерация через Hugging Face Space FLUX.2-dev-turbo
    ...
    # изображение — это dict с ключом 'path'
    image_dict = result[0]
    output_path = image_dict['path']
...
async def style_handler(callback: CallbackQuery):
    ...
    try:
        ...
        result_path = await loop.run_in_executor(None, generate_image_flux, photo_path, prompt)
        ...
        if "queue" in error_text.lower():
            ...
            await callback.message.answer(
                ...
                f"Попробуй ещё раз или отправь другое фото."
            )
