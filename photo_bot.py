...
async def generate_image_huggingface(image_path: str, prompt: str) -> bytes:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            async with session.post(HF_MODEL_URL, headers=headers, data=data) as response:
                ...
                if response.status != 200:
                    try:
                        ...
                        error_text = "Не удалось прочитать текст ошибки от Hugging Face"
                ...
                if "image" not in content_type:
                    try:
                        ...
                        text = "Hugging Face вернул ответ не в формате изображения"
            ...
            raise Exception(
                "Hugging Face вернул неполный ответ. Такое иногда бывает при перегрузке модели. Попробуй ещё раз через минуту."
            )
        except asyncio.TimeoutError:
            raise Exception(
                "Время ожидания ответа от Hugging Face истекло. Попробуй ещё раз чуть позже."
            )
...
async def style_handler(callback: CallbackQuery):
    ...
    try:
        ...
        if "503" in error_text:
            ...
            safe_error = html.escape(error_text[:700])
            await callback.message.answer(
